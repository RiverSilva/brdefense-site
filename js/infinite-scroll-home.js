// Rolagem Infinita para Homepage - BR Defense Center
// Baseado na implementação da página News, adaptado para a home

(function() {
    'use strict';

    // Configuração específica para a homepage
    const config = {
        debug: true,
        buttonText: 'Carregar Mais Notícias',
        loadingText: 'Carregando...',
        noMoreText: 'Não há mais notícias',
        scrollThreshold: 100, // pixels antes do final
        fadeInDuration: 600,
        retryAttempts: 3,
        retryDelay: 1000
    };
    ...
    const state = {
        currentPage: 1,
        totalPages: 4, // Extraído do HTML "1/4"
        loading: false,
        hasMore: true,
        retryCount: 0
    };
    ...
    function log(...args) {
        if (config.debug) {
            console.log('[Home Infinite Scroll]', ...args);
        }
    }

    // Inicialização
    function init() {
        log('Inicializando rolagem infinita na homepage...');
        
        // Verifica se estamos na homepage
        if (!isHomePage()) {
            log('Não é a homepage, saindo...');
            return;
        }
    ...
    function initializeElements() {
        // Container principal dos posts (main.list)
    ...
    async function loadMorePosts() {
        ...
        try {
            ...
            // URL da próxima página (homepage tem padrão /page/X/)
            const nextPageUrl = `/page/${nextPage}/`;
